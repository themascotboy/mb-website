...
export const ProjectManager = () => {
  ...
  const [formData, setFormData] = useState({ title: '', description: '', imageUrl: '', status: 'Active' });
  ...
  const handleEdit = (item: any) => {
    ...
    setFormData({ title: item.title, description: item.description, imageUrl: item.imageUrl || '', status: item.status || 'Active' });
    setIsOpen(true);
  };

  const handleNew = () => {
    setEditingId(null);
    setFormData({ title: '', description: '', imageUrl: '', status: 'Active' });
    setIsOpen(true);
  };
